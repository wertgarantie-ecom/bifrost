...
exports.insert = async function insert(clientData) {
    ...
    try {
        ...
        const query = {
            name: 'insert-client',
            text: "INSERT INTO client (id, name, email, backends, activePartnerNumber, basicauthuser, basicauthpassword, handbook) VALUES ($1 , $2, $3, $4, $5, $6, $7, $8);",
            values: [
                clientData.id,
                clientData.name,
                clientData.email,
...
exports.findClientForSecret = async function findClientForSecret(secret) {
    ...
    const result = await pool.query({
        ...
        text: `SELECT c.id, c.name, c.email, c.backends, c.activepartnernumber, c.basicauthuser, c.basicauthpassword, c.handbook, ARRAY_AGG(DISTINCT(cs.secret)) secrets, ARRAY_AGG(DISTINCT(cp.publicid)) publicids FROM client c 
...
exports.findAllClients = async function findAllClients() {
    ...
    const result = await pool.query({
        ...
        text: `SELECT c.id, c.name, c.email, c.backends, c.activepartnernumber, c.basicauthuser, c.basicauthpassword, c.handbook, ARRAY_AGG(DISTINCT(cs.secret)) secrets, ARRAY_AGG(DISTINCT(cp.publicid)) publicids
                FROM client c
                INNER JOIN clientsecret cs on c.id = cs.clientid
                INNER JOIN clientpublicid cp on c.id = cp.clientid
                GROUP By c.id`
