...
exports.getAfterSalesData = async function getAfterSalesData(req, res, next) {
    ...
    try {
        const result = await afterSalesService.showAfterSalesComponent(sessionId, req.clientConfig, req.locale.language);
        if (!result) {
...
exports.componentCheckout = async function componentCheckout(req, res, next) {
    try {
        const result = await afterSalesService.checkoutAndShowAfterSalesComponent(req.shoppingCart, req.clientConfig, req.body.webshopData);
